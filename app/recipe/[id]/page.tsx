...
import { ArrowLeft, Clock, Users, Zap, Bookmark, BookmarkCheck, ExternalLink, CheckCircle2, Apple } from "lucide-react";
...
export default function RecipePage() {
  ...
  if (!recipe) {
    return (
      <>
        <Navbar />
        <div style={{ maxWidth: "576px", margin: "0 auto", padding: "80px 24px", textAlign: "center" }}>
          ...
          <button onClick={() => router.push("/")} className="px-6 py-3 rounded-xl text-white font-semibold"
            style={{ backgroundColor: "var(--forest)", border: "none", cursor: "pointer" }}>
            Back to Search
          </button>
        </div>
      </>
    ...
    <>
      ...
      <main style={{ maxWidth: "1024px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        <button onClick={() => router.back()}
          ...
          style={{ color: "var(--soft-brown)", background: "none", border: "none", cursor: "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "32px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            {/* Image */}
            <div style={{ position: "relative", borderRadius: "20px", overflow: "hidden", aspectRatio: "1/1" }}>
              {recipe.image ? (
                <Image src={recipe.image} alt={recipe.label} fill style={{ objectFit: "cover" }} sizes="(max-width: 1024px) 100vw, 40vw" />
              ) : (
                <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "60px", backgroundColor: "var(--border)" }}>🍽️</div>
              )}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              {nutrients.map((n) => (
                <div key={n.label} style={{ borderRadius: "12px", padding: "14px", border: `1px solid var(--border)`, backgroundColor: "white", textAlign: "center" }}>
                  <p className="font-display text-2xl font-bold" style={{ color: n.color }}>{n.value}</p>
                  <p className="text-xs mt-1" style={{ color: "var(--soft-brown)" }}>{n.label} ({n.unit})</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            ...
            <div>
              ...
              <h1 className="font-display font-bold leading-tight mt-1" style={{ fontSize: "clamp(1.75rem, 4vw, 2.5rem)", color: "var(--charcoal)" }}>
                {recipe.label}
              </h1>
              ...
              <div className="flex flex-wrap gap-4 mt-4 text-sm">
                {recipe.totalTime > 0 && (
                  <span className="flex items-center gap-1.5" style={{ color: "var(--charcoal)" }}>
                    <Clock size={16} style={{ color: "var(--sage)" }} /> {recipe.totalTime} minutes
                  </span>
                )}
                <span className="flex items-center gap-1.5" style={{ color: "var(--charcoal)" }}>
                  <Users size={16} style={{ color: "var(--sage)" }} /> {recipe.yield} servings
                </span>
                <span className="flex items-center gap-1.5" style={{ color: "var(--charcoal)" }}>
                  <Zap size={16} style={{ color: "var(--terracotta)" }} /> {cal} kcal / serving
                </span>
              </div>
              ...
              <div className="flex gap-3 mt-5 flex-wrap">
                <button onClick={toggleSave}
                  ...
                  style={saved
                    ? { backgroundColor: "var(--forest)", color: "white", border: "none", cursor: "pointer" }
                    : { backgroundColor: "white", color: "var(--charcoal)", border: "1.5px solid var(--border)", cursor: "pointer" }}>
                  {saved ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}
                  {saved ? "Saved" : "Save Recipe"}
                </button>
                <a href={recipe.url} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition-all hover:opacity-90"
                  style={{ backgroundColor: "var(--terracotta)", textDecoration: "none" }}>
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-display font-semibold text-xl" style={{ color: "var(--charcoal)" }}>Ingredients</h3>
                <span className="text-xs" style={{ color: "var(--soft-brown)" }}>(tap to check off)</span>
              </div>
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "8px" }}>
                {recipe.ingredientLines.map((line, i) => (
                  <li key={i} onClick={() => toggleIngredient(i)}
                    style={{ display: "flex", alignItems: "flex-start", gap: "12px", fontSize: "14px", cursor: "pointer", padding: "4px 0", color: checkedIngredients.has(i) ? "var(--sage)" : "var(--charcoal)" }}>
                    <CheckCircle2 size={18} style={{ marginTop: "2px", flexShrink: 0, color: checkedIngredients.has(i) ? "var(--sage)" : "var(--border)", transition: "color 0.2s" }} />
                    <span style={{ textDecoration: checkedIngredients.has(i) ? "line-through" : "none", opacity: checkedIngredients.has(i) ? 0.5 : 1, transition: "all 0.2s" }}>
                      {line}
                    </span>
                  </li>
                ))}
